...
## Before Next Class {.tabset} 
 - Complete Lab Assignment 1
 - Read Chapters 4 through 8 (except for the last section of each chapter) of [YaRrr! The Pirate's Guide to R](https://bookdown.org/ndphillips/YaRrr/basics.html) before the next class
 - Complete the Lab-2-Tutorial posted in Canvas
...
You can also pull up the Markdown Quick Reference guide in the help window of the R Studion GUI. Go to Help --> Markdown Quick Refence 
...
Full Disclosure *Much of this section comes from Jonathan Gilligan's introduction to Reproducible Research which can be found [here](https://github.com/gilligan-ees-3310-2020/lab-01-documentation).
  ...
  You should now have in the upper-right window a tab that says "Git". This window will show you a list of all the files in your R PRoject that have been modified. If we click   on the "Committ" button we should be able to see what has been changed in each document as well as a history of these changes. Right now you won't see anything becuase you haven't changed any files in your R Project since we enabled version control.
...
- To keep things simple let's start by just making a few variables to work with. Type the following code in your R Script, then select the lines of code you want to run and hit **cntrl+Enter**.
...
1. Create an R Project for your lab assignment with version control (already done)
...
7. Your R Markdown file should include text describing what you are doing, code chunks where you run actual R code, and text describing the output
8. Use headers and include at least two examples of non-standard text formatting, and one list in your file
9. Knit your R Markdown file and upload a copy of the knit .html and the .Rmd file to Canvas
